    ...
    def __init__(self, id, sp, ds, gridLength, metals=None, star_SL_func=None):
        ...
        while True:
            ...
            for i in range(len(__gPartCoord)):
                ...
                __denGrid = __gPartMass[i]* __gaussGrid / dVcell
                for mi in range(len(__metalArr)):
                    self.gasMetalMetallicityGrids[__metalArr[mi]] = self.gasMetalMetallicityGrids[__metalArr[mi]] + (__massGrid * 1e-6) # test case all metallicities at 1e-6 instead of __gPartZarr[mi][i])
                self.gasDensityGrid = self.gasDensityGrid + __denGrid
                self.gasTemperatureGrid = self.gasTemperatureGrid + __mT
    ...
    def __init__(self, ds, skidcat, snapname, nproc, fstar, deltac, grid_length, metals=None, star_SL_func=None, filedirname=None):
        ...
        if nproc == 1:
            ...
            for i in range(totGalNum): 
                ...
                print(f"HaloGridDataset complete: {int(counter)}/{totGalNum}", end='\r', flush=True)
                counter += 1
            ...
            def ggproc(idL, gridL, skidIDArr, skidMstarArr, ds, grid_length, metals, star_SL_func, counter):
                for i in range(len(skidIDArr)):
                    ...
                    sp = ds.sphere(center, r_s)
                    idL.append(skidIDArr[i])
                    temp = VirialGrid(skidIDArr[i], sp, ds, grid_length, metals, star_SL_func)
                    gridL.append(temp)
                    counter.value += 1
