...
def __save_hdf5__(table, fname, catmode, ion_i_lines):
    """Save to HDF5
    ...
    for snapi in range(n_i,n_f+1): # for each snapshot and associated data
            ...
            for ionName in ion_lines:
                if ionName in sys_abs:
                    ...
                    try:
                        ...
                        if type(sysVel) == np.float64:
                            sysVel = np.array([sysVel])
        ...
        if write: # if you want to write it out
            if savename == None:
                ...
                fname = savename
            __save_hdf5__(refTable, fname, catmode, ion_i_lines)
